...
fn main() {
    ...
    if args.len() != 2 {
        eprintln!("Usage: {} <magnification factor>x", args[0]);
        std::process::exit(1);
    }
    ...
    let mut img = RgbImage::new((width * magnification) as u32, (height * magnification) as u32);
        ...
        for (x, c) in line.chars().enumerate() {
            ...
            for dy in 0..magnification {
                for dx in 0..magnification {
                    img.put_pixel((x * magnification + dx) as u32, (y * magnification + dy) as u32, color);
                }
            }
        }
    }
    ...
    let mut encoder = Encoder::new(&mut handle, (width * magnification) as u32, (height * magnification) as u32);
